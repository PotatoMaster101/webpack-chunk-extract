...
public static class ArgHelper
{
    ...
    public static async Task HandleFileArg(string file, bool verbose)
    {
        ...
        var loaders = GetLoaders(content);
    ...
    public static async Task HandleSiteArg(string url, bool verbose)
    {
        foreach (var jsUrl in await GetJsUrls(url, verbose))
        {
            ...
            foreach (var loader in GetLoaders(content))
                PrintLoader(jsUrl, loader);
        }
    }

    /// <summary>
    /// Returns the list of chunk loaders from JS content.
    /// </summary>
    /// <param name="content">The JS content.</param>
    /// <returns>The list of chunk loaders.</returns>
    private static IEnumerable<Loader> GetLoaders(string content)
    {
        var parser = new Parser();
        var program = parser.ParseModule(content);
        return program.GetLoaders();
    }
    ...
    private static void PrintLoader(string url, Loader loader)
    {
        var chunkIds = loader.GetChunkIds();
        if (chunkIds.Count == 0)
            return;
