...
## Note
- This tool requires [Playwright browsers](https://playwright.dev/docs/browsers): `npx playwright install`
- This tool only searches for chunk loaders when visiting a Webpack site's home page, hence it might miss some chunk loaders loaded on demand (such as when navigating)
  - To search more thorougly use a tool like [Burp](https://portswigger.net/burp) to dump all the `.js` files encountered to a folder then run the tool use `-d`
