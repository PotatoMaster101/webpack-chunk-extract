...
{
    [Theory]
    [ClassData(typeof(GetChunkIdsTestData))]
    public void GetChunkIds_ReturnsCorrectValue(string js, ChunkId[] expected)
    {
        // arrange
        var parser = new Parser();
        var module = parser.ParseModule(js);
        var loaders = ChunkLoader.FromRootNode(module).ToList();

        // act
        var result = loaders.SelectMany(x => x.GetChunkIds()).ToList();
    ...
    [Theory]
    [ClassData(typeof(RunTestData))]
    public void Run_ReturnsCorrectValue(string js, ChunkEntry[] expected)
    {
        // arrange
        var parser = new Parser();
        var module = parser.ParseModule(js);
        ...
        // act
        var result = loaders.SelectMany(x => x.Run(x.GetChunkIds())).ToList();
    ...
    [Theory]
    [ClassData(typeof(FromRootNodeTestData))]
    public void FromRootNode_ReturnsCorrectValue(string js, int expectedCount)
    {
        // arrange
        var parser = new Parser();
        var module = parser.ParseModule(js);
        ...
        // assert
        Assert.Equal(expectedCount, loaders.Count);
    }

    /// <summary>
    /// Unit tests for <see cref="ChunkLoaderTest.GetChunkIds_ReturnsCorrectValue"/>.
    /// </summary>
    private class GetChunkIdsTestData : IEnumerable<object[]>
    {
    ...
    /// <summary>
    /// Unit tests for <see cref="ChunkLoaderTest.Run_ReturnsCorrectValue"/>.
    /// </summary>
    private class RunTestData : IEnumerable<object[]>
    {
        IEnumerator IEnumerable.GetEnumerator() => GetEnumerator();
    ...
    /// <summary>
    /// Unit tests for <see cref="ChunkLoaderTest.FromRootNode_ReturnsCorrectValue"/>.
    /// </summary>
    private class FromRootNodeTestData : IEnumerable<object[]>
    {
        IEnumerator IEnumerable.GetEnumerator() => GetEnumerator();
        public IEnumerator<object[]> GetEnumerator()
        {
